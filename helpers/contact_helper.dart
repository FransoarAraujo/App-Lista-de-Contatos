...
class ContactHelper {
  ...
  Future<Database> initDb() async {
    ...
    return await openDatabase(path, version: 1,
      ...
      await db.execute(
          "CREATE TABLE $contactTable ($idColumn PRIMARY KEY, $nomeColumn TEXT, $emailColumn TEXT, $phoneColumn TEXT, $imageColumn TEXT)");
    });
  }
  ...
  Future<Contact> getContact(int id) async {
    ...
    List<Map> contactsMap = await contactDb.query(
      contactTable,
      columns: [idColumn, nomeColumn, emailColumn, phoneColumn, imageColumn],
      where: "$idColumn = ?",
      whereArgs: [id],
    );
...
class Contact {
  ...
  Contact.empty();
  Contact(this.id, this.nome, this.email, this.phone, this.image);

  Contact.fromMap(Map map) {
    Contact(map[idColumn], map[nomeColumn], map[emailColumn], map[phoneColumn],
        map[imageColumn]);
  }
  ...
  @override
  String toString() {
    return "Contact(id: $id, nome: $nome, email: $email, phone: $phone, image: $image)";
  }
}
